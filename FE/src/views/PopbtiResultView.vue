...
const point = ref(route.params.mbtiId)
...
const setResult = () => {
  ...
  // console.log(point + '%%')
  resultName.innerHTML = infoList[point.value].name;

  var resultImg = document.createElement('img');
  const imgDiv = document.querySelector('#resultImg');
  var imgURL = '/src/assets/images/popbti-img/popbti-img-' + point.value + '.svg';
  
  console.log(imgURL)
  resultImg.src = imgURL;
  resultImg.alt = point.value;
  resultImg.classList.add('img-fluid');
  imgDiv.appendChild(resultImg);


  const resultDesc = document.querySelector('.resultDesc');
  resultDesc.innerHTML = infoList[point.value].desc;
}

onMounted(() => {
  const route = useRoute();
  console.log(route.params.mbtiId);
